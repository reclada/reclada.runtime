...
aws s3 cp "${_OUTPUT_DIR}/" "s3://${AWS_S3_BUCKET_NAME}/output/${_JOB_ID}/context/output/" --recursive --sse
aws s3 cp "${_INPUT_DIR}/" "s3://${AWS_S3_BUCKET_NAME}/output/${_JOB_ID}/context/input/" --recursive --sse
